...
// Generic auth button component that can be styled for different providers
export const AuthButton = ({ 
  ...
  const getProviderStyles = () => {
    switch (provider.id) {
      ...
      case 'apple':
        return 'bg-black hover:bg-gray-900 text-white border border-gray-800';
      case 'facebook':
  ...
  return (
    <button
      ...
      className={`w-full font-medium py-2 px-6 rounded-full transition-colors duration-200 flex items-center justify-center space-x-4 disabled:opacity-50 disabled:cursor-not-allowed text-lg shadow-lg ${getProviderStyles()} ${className}`}
    >
      {loading ? (
